...
/**
 * MCP tool for retrieving product information for a specific account
 * from the Open Banking API.
 *
 * <p>Returns typed {@link OBReadProduct2} records on success. Spring AI generates
...
public class ProductTools {
	...
	public ProductTools(OpenBankingClient openBankingClient, ObjectMapper objectMapper) {
		this.openBankingClient = openBankingClient;
		this.objectMapper = objectMapper;
	}

	@McpTool(name = "get_product", description = "Retrieve product information for a specific account by AccountId")
	public OBReadProduct2 getProduct(
		...
		try {
			String response = openBankingClient.get("/accounts/" + accountId + "/product");
			String json = extractJsonBody(response);
			return objectMapper.readValue(json, OBReadProduct2.class);
